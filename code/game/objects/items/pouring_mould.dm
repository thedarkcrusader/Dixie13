...
/obj/item/mould/examine(mob/user)
	...
	if(fufilled_metal)
		...
		. += "[src] has [fufilled_metal] drops of <font color=[reagent_color]> Molten [initial(filling_metal.name)]</font> out of [round(required_metal / 3, 1)] oz.</font>"
	else
		. += "[src] requires [required_metal] drops of Molten Metal to form.</font>"

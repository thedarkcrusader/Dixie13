...
/obj/structure/handle_mouseover(location, control, params)
	...
	if(p.client)
		...
		if((((rotation_structure && rotation_network) || istype(src, /obj/structure/water_pipe)) || accepts_water_input) && HAS_TRAIT(p, TRAIT_ENGINEERING_GOGGLES))
			...
			p.client.mouseovertext.maptext_width = 96
			p.client.mouseovertext.maptext = MAPTEXT({"[rotation_chat]\
			...
			p.client.mouseovertext.maptext_height = 32
			p.client.mouseovertext.maptext_width = 96
			p.client.mouseovertext.maptext = MAPTEXT({"<span style='font-size:8pt;font-family:"Pterra";color:[hover_color];text-shadow:0 0 10px #fff, 0 0 20px #fff, 0 0 30px #e60073, 0 0 40px #e60073, 0 0 50px #e60073, 0 0 60px #e60073, 0 0 70px #e60073;' class='center maptext '>[name]"})
		p.client.screen |= p.client.mouseovertext
	return TRUE
...
/turf/handle_mouseover(location,control,params)
	...
	if(p.client)
		...
		if(offset_x < 1 || offset_x > 15 || offset_y < 1 || offset_x > 15)
			return FALSE
		var/list/PM = list("screen-loc" = "[offset_x]:0,[offset_y]:0")
		p.client.mouseovertext.maptext = MAPTEXT({"<span style='font-size:8pt;font-family:"Pterra";color:#607d65;text-shadow:0 0 10px #fff, 0 0 20px #fff, 0 0 30px #e60073, 0 0 40px #e60073, 0 0 50px #e60073, 0 0 60px #e60073, 0 0 70px #e60073;' class='center maptext '>[name]"})
		p.client.mouseovertext.movethis(PM)
		p.client.screen |= p.client.mouseovertext
	return TRUE
...
/turf/open/handle_mouseover(location, control, params)
	...
	if(p.client)
		...
		var/offset_y = 8 - (p.y - y)
		var/list/PM = list("screen-loc" = "[offset_x]:0,[offset_y]:0")
		p.client.mouseovertext.maptext = MAPTEXT({"<span style='font-size:8pt;font-family:"Pterra";color:#6b3f3f;text-shadow:0 0 10px #fff, 0 0 20px #fff, 0 0 30px #e60073, 0 0 40px #e60073, 0 0 50px #e60073, 0 0 60px #e60073, 0 0 70px #e60073;' class='center maptext '>[name]"})
		p.client.mouseovertext.movethis(PM)
		p.client.screen |= p.client.mouseovertext
	return TRUE
...
/mob/handle_mouseover(location,control,params)
	...
	if(p.client)
		...
		p.client.mouseovertext.maptext = MAPTEXT({"<span style='font-size:8pt;font-family:"Pterra";color:[mousecolor];text-shadow:0 0 10px #fff, 0 0 20px #fff, 0 0 30px #e60073, 0 0 40px #e60073, 0 0 50px #e60073, 0 0 60px #e60073, 0 0 70px #e60073;' class='center maptext '>[name]"})
		p.client.mouseovertext.movethis(PM)
		p.client.screen |= p.client.mouseovertext
	return TRUE

/datum/ai_behavior/lampreywolf_melee_attack
	action_cooldown = 0.2 SECONDS // We gotta check unfortunately often because we're in a race condition with nextmove
	behavior_flags = AI_BEHAVIOR_REQUIRE_MOVEMENT | AI_BEHAVIOR_REQUIRE_REACH | AI_BEHAVIOR_CAN_PLAN_DURING_EXECUTION
	var/sidesteps_after = FALSE

/datum/ai_behavior/lampreywolf_melee_attack/setup(datum/ai_controller/controller, target_key, targetting_datum_key, hiding_location_key)
	. = ..()
...
/datum/ai_behavior/lampreywolf_melee_attack/perform(delta_time, datum/ai_controller/controller, target_key, targetting_datum_key, hiding_location_key)
	. = ..()
	...
	if(basic_mob.next_click == world.time + 1 && istype(basic_mob)) //oopsie I hate that roguecode fucks change_nextMove
		basic_mob.next_click = world.time + basic_mob.melee_attack_cooldown
		SEND_SIGNAL(basic_mob, COMSIG_MOB_BREAK_SNEAK)
		controller.set_blackboard_key(BB_LAMPREYWOLF_NEXTATTACKING, basic_mob.next_click)
...
/datum/ai_behavior/lampreywolf_melee_attack/finish_action(datum/ai_controller/controller, succeeded, target_key, targetting_datum_key, hiding_location_key)
	. = ..()
	if(!succeeded)

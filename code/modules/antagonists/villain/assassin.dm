...
/datum/antagonist/assassin/on_gain()
	...
	if(owner.current.job != "Drifter") // This code only runs if the assassin is assigned midround and is not a drifter.
		...
		var/wah = /obj/item/weapon/garrote/razor
		...
		to_chat(owner.current, "<span class='danger'>I've blended in well up until this point, but it's time for the Hunted of Graggar to perish. I must get my tools from where I hid it.</span>")
	return ..()

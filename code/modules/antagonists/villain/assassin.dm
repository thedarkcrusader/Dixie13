...
/datum/antagonist/assassin/on_gain()
	...
	if(owner.current.job != "Drifter") // This code only runs if the assassin is assigned midround and is not a drifter.
		...
		if(old_grab_skill < 4) // If the assassined player has less than 4 wrestling skill, get them to 4.
			owner.current.adjust_skillrank(/datum/skill/combat/wrestling, 4 - old_grab_skill, TRUE)

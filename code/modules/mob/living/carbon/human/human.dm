	...
	if(is_short_blade(held_item))
		if(user.cmode)
			if(user.zone_selected == BODY_ZONE_PRECISE_NECK)
				try_throat_slit(user, held_item, params)
		else
			if(user.zone_selected == BODY_ZONE_PRECISE_MOUTH)
				try_shave(user, held_item, params)

/mob/living/carbon/human/proc/try_throat_slit(mob/user, obj/item/used_item, params)
	playsound(src, 'sound/surgery/scalpel1.ogg', 100, TRUE, -1)
	var/possessive = (user == src) ? user.p_their() : "\the [src]'s"
	user.visible_message(span_danger("[user] starts to slit [possessive] throat with [used_item]."))
	if(do_after(user, 5 SECONDS, src))
		var/obj/item/bodypart/part = src.get_bodypart(BODY_ZONE_PRECISE_NECK)
		part.add_wound(/datum/wound/artery/neck)

/mob/living/carbon/human/proc/try_shave(mob/user, obj/item/used_item, params)
	...
	if(has_stubble)
		...
		user.visible_message(span_danger("[user] starts to shave [possessive] stubble with [used_item]."))
		if(do_after(user, 5 SECONDS, src))
			has_stubble = FALSE
			update_body()
	else if(facial?.accessory_type != /datum/sprite_accessory/hair/facial/none)
		playsound(src, 'sound/foley/shaving.ogg', 100, TRUE, -1)
		user.visible_message(span_danger("[user] starts to shave [possessive] facehairs with [used_item]."))
		if(do_after(user, 5 SECONDS, src))
			set_facial_hair_style(/datum/sprite_accessory/hair/facial/none)
			update_body()
			record_round_statistic(STATS_BEARDS_SHAVED)
			if(isdwarf(src))
				add_stress(/datum/stress_event/dwarfshaved)
	// if we canceled early, do the attack chain
	used_item.melee_attack_chain(user, src, params)

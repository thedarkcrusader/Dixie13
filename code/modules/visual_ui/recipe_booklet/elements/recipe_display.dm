...
/obj/abstract/visual_ui_element/scrollable/selected_recipe/proc/build_recipe()
	...
	for(var/datum/reagent/path as anything in recipe.reagent_requirements)
		...
		register_element(requirement)
		length++
		requirement.maptext = {"<span style='font-size:8pt;font-family:"Pixelify Sans";color:[hover_color]' class='center maptext '>[CEILING(recipe.reagent_requirements[path], 1)] drops of [initial(path.name)]</span>"}
